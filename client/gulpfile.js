  ...
  return (
    src(path.src.html) // Путь до исходных файлов в src
      ...
      .pipe(
        ...
        inject(src(['./dist/styles/*.css'], { read: false }), {
      ...
      .pipe(
        // Читаем имена файлов с хэшем
        inject(
          src(['./dist/scripts/main-*.js'], { read: false }),
...
// Собрать проект
export const build = series(clean, style, js, image, fonts, html)

// По дефолту всё собираем и запускаем сервер
const _default = series(style, js, image, fonts, html, function () {
  browserSync(config)
  watch(path.watch.html, series(html))
  watch(path.watch.sass, series(style, html))
  watch(path.watch.libs_sass, series(style, html))
  watch(path.watch.js, series(js, html))
  watch(path.watch.libs_js, series(js, html))
